...
$cur = $pdo->prepare("SELECT type FROM categories WHERE id=:id");
$cur->execute([':id'=>$id]);
$type = $cur->fetchColumn() ?: 'maquillage';

$check = $pdo->prepare("SELECT COUNT(*) FROM services WHERE category_id = :id");
$check->execute([':id' => $id]);
...
header('Location: changeTarifs.php?section='.$type); // adapte le chemin si besoin
exit;